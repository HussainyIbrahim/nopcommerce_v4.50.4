...
using Nop.Web.Models.Tamayouz.Request;
using Nop.Web.Models.Tamayouz.Response;
...
namespace Nop.Web.Controllers
{
    ...
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        /// <summary>
        /// generate token from tamayouz
        /// </summary>
        /// <returns>a task of string that represents the token</returns>
        private async Task<string> Authentication()
        {
            #region intialize response
            var authenticationResponse = new AuthenticationResponse();
            #endregion

            #region configs
            var baseURL = _appSettings.Get<TamayouzAPIConfig>().BaseURL;
            var authenticationEndPoint = _appSettings.Get<TamayouzAPIConfig>().Authentication;
            var accountIdentity = _appSettings.Get<TamayouzAPIConfig>().AccountIdentity;
            var password = _appSettings.Get<TamayouzAPIConfig>().Password;
            var timeZone = _appSettings.Get<TamayouzAPIConfig>().TimeZone;
            #endregion

            #region call the api
            var httpRequestMessage = new HttpRequestMessage(HttpMethod.Post, baseURL + authenticationEndPoint) { };
            #region set Authentication API headers
            httpRequestMessage.Headers.Add(HeaderNames.AcceptLanguage, "en-US");
            httpRequestMessage.Headers.Add(HeaderNames.Accept, "application/json");
            httpRequestMessage.Headers.Add("api-version", "v1");
            httpRequestMessage.Headers.Add("GlobalId", Guid.NewGuid().ToString());
            #endregion

            #region set the request body
            httpRequestMessage.Content = new StringContent(JsonSerializer.Serialize(new AuthenticationRequest()
            {
            ...
            #region send the request with body
            var response = await _httpClient.SendAsync(httpRequestMessage);
            #endregion

            #region if the request successedded
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                #region map success response
                var content = await response.Content.ReadAsStringAsync();
                authenticationResponse = JsonSerializer.Deserialize<AuthenticationResponse>(content); 
                #endregion

                #region set token in cache and set caching time
                DateTime utcTime = DateTime.UtcNow;
                TimeZoneInfo serverZone = TimeZoneInfo.FindSystemTimeZoneById(timeZone);
                DateTime currentDateTime = TimeZoneInfo.ConvertTimeFromUtc(utcTime, serverZone);
                var cacheKey = new CacheKey("TamayouzToken");
                cacheKey.CacheTime = ((int)authenticationResponse.tokenExpirationDate.TimeOfDay.TotalMinutes) - ((int)currentDateTime.TimeOfDay.TotalMinutes); 
                await _staticCacheManager.SetAsync(cacheKey, authenticationResponse.token);
                #endregion
            }
            #endregion
            #region if the request failed
            else
            {
                #region map fail response
                var content = await response.Content.ReadAsStringAsync();
                var failResponse = JsonSerializer.Deserialize<FailResponse>(content); 
                #endregion
            }
            #endregion
        ...
        [HttpPost, ActionName("Cart")]
        [FormValueRequired("sendOTP")]
        public virtual async Task SendOTP(string mobile, IFormCollection form)
        {
            var cacheKey = new CacheKey("TamayouzToken");
            var token = await _staticCacheManager.GetAsync(cacheKey, Authentication);
            #region intialize response
            var authenticationResponse = new AuthenticationResponse();
            #endregion
            ...
            #region configs
            var baseURL = _appSettings.Get<TamayouzAPIConfig>().BaseURL;
            var sendOTPEndPoint = _appSettings.Get<TamayouzAPIConfig>().OTP;
            var branchId = _appSettings.Get<TamayouzAPIConfig>().BranchId;
            #endregion

            #region call the api
            var httpRequestMessage = new HttpRequestMessage(HttpMethod.Post, baseURL + sendOTPEndPoint) { };
            #region set Authentication API headers
            httpRequestMessage.Headers.Add(HeaderNames.AcceptLanguage, "en-US");
            httpRequestMessage.Headers.Add(HeaderNames.Accept, "application/json");
            httpRequestMessage.Headers.Add("api-version", "v1");
            ...
            #region set the request body
            httpRequestMessage.Content = new StringContent(JsonSerializer.Serialize(new SendOTPRequest()
            {
                BranchId = "",
                Msisdn = mobile,
            ...
            #region map if the request successedded
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                var content = await response.Content.ReadAsStringAsync();
                authenticationResponse = JsonSerializer.Deserialize<AuthenticationResponse>(content);
            ...
            {
                await _staticCacheManager.GetAsync(new CacheKey("TamayouzToken"), Authentication);
            }
            else
            {
                var content = await response.Content.ReadAsStringAsync();
